...
<div class="max-w-xl mx-auto p-6 bg-white shadow-md rounded">
   ...
   <form action="{{ route('transacciones.update', $transaccion['id']) }}" method="POST" class="space-y-4">
    @csrf
    @method('PUT')

    <div>
        <label for="descripcion" class="block text-sm font-medium text-gray-700">Descripción:</label>
        <input type="text" name="descripcion" id="descripcion" value="{{ old('descripcion', $transaccion['description']) }}"
    ...
    <div>
        ...
        <input type="number" name="monto" id="monto" value="{{ old('monto', $transaccion['amount']) }}"
            class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
            required>
        @error('monto')
        <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
    ...
    <div>
        ...
        <input type="date" name="fecha_transaccion" id="fecha_transaccion"
            value="{{ old('fecha_transaccion', $transaccion['transaction_date']) }}"
            class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
